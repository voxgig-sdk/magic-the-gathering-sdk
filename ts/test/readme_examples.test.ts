...
//     (\`new MagicTheGatheringSDK(...)\` / \`MagicTheGatheringSDK.test(...)\` become
//     \`MagicTheGatheringSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'MagicTheGatheringSDK'
...
const TEST_SEED = {"entity":{"card":{"test01":{"id":"test01"}},"set":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MagicTheGatheringSDK(...)\` and any
// \`MagicTheGatheringSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MagicTheGatheringSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MagicTheGatheringSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
