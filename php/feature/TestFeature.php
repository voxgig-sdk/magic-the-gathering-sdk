...
// MagicTheGathering SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class MagicTheGatheringTestFeature extends MagicTheGatheringBaseFeature
{
    ...
    public function init(MagicTheGatheringContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (MagicTheGatheringContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = MagicTheGatheringParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (MagicTheGatheringContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(MagicTheGatheringContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = MagicTheGatheringParam::call($ctx, $k);
