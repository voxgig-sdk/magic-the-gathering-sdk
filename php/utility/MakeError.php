...
class MagicTheGatheringMakeError
{
    public static function call(?MagicTheGatheringContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MagicTheGatheringContext([], null);
        }
        $op = $ctx->op ?? new MagicTheGatheringOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MagicTheGatheringResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MagicTheGatheringError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MagicTheGatheringError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof MagicTheGatheringError) {
