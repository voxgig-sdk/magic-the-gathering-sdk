...
class CardEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = MagicTheGatheringSDK::test($seed, null);
        $seen = iterator_to_array($base->Card(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = MagicTheGatheringConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = MagicTheGatheringSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAGIC_THE_GATHERING_TEST_CARD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function card_basic_setup($extra)
{
    ...
    $client = MagicTheGatheringSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("MAGIC_THE_GATHERING_TEST_CARD_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["MAGIC_THE_GATHERING_TEST_CARD_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["MAGIC_THE_GATHERING_TEST_LIVE"] === "TRUE") {
        ...
        $client = new MagicTheGatheringSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["MAGIC_THE_GATHERING_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
