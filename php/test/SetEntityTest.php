...
class SetEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAGICTHEGATHERING_TEST_SET_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function set_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("MAGICTHEGATHERING_TEST_SET_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "MAGICTHEGATHERING_TEST_SET_ENTID" => $idmap,
        "MAGICTHEGATHERING_TEST_LIVE" => "FALSE",
        "MAGICTHEGATHERING_TEST_EXPLAIN" => "FALSE",
    ]);

    $idmap_resolved = Helpers::to_map(
        $env["MAGICTHEGATHERING_TEST_SET_ENTID"]);
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["MAGICTHEGATHERING_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
