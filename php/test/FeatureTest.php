...
// MagicTheGathering SDK feature test
//
...
class FtCtrl extends MagicTheGatheringControl
{
...
class FtClient
{
    ...
    private MagicTheGatheringUtility $utility;

    public function __construct(MagicTheGatheringUtility $utility, array $options)
    {
    ...
    public function get_utility(): MagicTheGatheringUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MagicTheGatheringContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MagicTheGatheringContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MagicTheGatheringContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MagicTheGatheringConfig::make_feature($name);
    ...
    private static function buildUrl(MagicTheGatheringSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MagicTheGatheringError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MagicTheGatheringResponse($fetched);
        }
    ...
    private function fail(MagicTheGatheringContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MagicTheGatheringContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
