...
//    only - a typed MagicTheGatheringContext there would make every ctx
...
final class MagicTheGatheringOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new MagicTheGatheringContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new MagicTheGatheringSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new MagicTheGatheringResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new MagicTheGatheringError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new MagicTheGatheringResponse($clean['response']);
    ...
    public static function err_from_map($m): ?MagicTheGatheringError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MagicTheGatheringError(is_string($code) ? $code : '', $msg);
    }
