// Command MagicTheGathering-cli is a CLI + REPL for the MagicTheGathering SDK,
...
const prompt = "magic-the-gathering"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "card set"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MAGIC_THE_GATHERING_APIKEY (api key), MAGIC_THE_GATHERING_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MAGIC_THE_GATHERING_APIKEY carries the API key and
	// MAGIC_THE_GATHERING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MAGIC_THE_GATHERING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MAGIC_THE_GATHERING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMagicTheGatheringSDK(opts)
