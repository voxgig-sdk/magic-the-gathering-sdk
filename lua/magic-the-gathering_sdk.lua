...
require("magic-the-gathering_types")
...
local MagicTheGatheringSDK = {}
MagicTheGatheringSDK.__index = MagicTheGatheringSDK
...
MagicTheGatheringSDK._make_feature = _make_feature


function MagicTheGatheringSDK.new(options)
  local self = setmetatable({}, MagicTheGatheringSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function MagicTheGatheringSDK:options_map()
...
function MagicTheGatheringSDK:get_utility()
  return Utility.copy(self._utility)
end


function MagicTheGatheringSDK:get_root_ctx()
  return self._rootctx
end


function MagicTheGatheringSDK:prepare(fetchargs)
...
function MagicTheGatheringSDK:direct(fetchargs)
...
function MagicTheGatheringSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function MagicTheGatheringSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "MagicTheGatheringSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function MagicTheGatheringSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MagicTheGatheringSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MagicTheGatheringSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function MagicTheGatheringSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = MagicTheGatheringSDK.new(sdkopts)
  sdk.mode = "test"
