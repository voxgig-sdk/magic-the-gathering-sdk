...
class TestCardEntity:
    ...
    def test_should_stream(self):
        ...
        base = MagicTheGatheringSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MAGIC_THE_GATHERING_TEST_CARD_ENTID JSON to run live")
        client = setup["client"]
...
def _card_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("MAGIC_THE_GATHERING_TEST_CARD_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("MAGIC_THE_GATHERING_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = MagicTheGatheringSDK(helpers.to_map(merged_opts))

    _live = env.get("MAGIC_THE_GATHERING_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("MAGIC_THE_GATHERING_TEST_EXPLAIN") == "TRUE",
        "live": _live,
