...
// MagicTheGathering SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = MagicTheGatheringOmni::makeStructRunner(
    ...
    {
        return MagicTheGatheringOmni::objmodel(MagicTheGatheringOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return MagicTheGatheringOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return MagicTheGatheringOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(MagicTheGatheringOmni::structfix($now, Struct::undef()))
                        !== json_encode(MagicTheGatheringOmni::structfix($val, Struct::undef()))) {
