...
// MagicTheGathering SDK retry feature
...
class MagicTheGatheringRetryFeature extends MagicTheGatheringBaseFeature
{
    ...
    public function init(MagicTheGatheringContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MagicTheGatheringContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MagicTheGatheringContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MagicTheGatheringContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
