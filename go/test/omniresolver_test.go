...
import (
	...
	omni "github.com/voxgig-sdk/magic-the-gathering-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MagicTheGatheringError); is {
				out := map[string]any{
					"name":    "MagicTheGatheringError",
...
func omniCtx(arg any, client *sdk.MagicTheGatheringSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MagicTheGatheringSDK); is {
			client = live
