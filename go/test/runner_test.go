...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MAGIC_THE_GATHERING_TEST_LIVE") == "TRUE" ||
		os.Getenv("MAGIC_THE_GATHERING_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MAGIC_THE_GATHERING_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.MagicTheGatheringSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.MagicTheGatheringSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.MagicTheGatheringError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.MagicTheGatheringSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.MagicTheGatheringError{Msg: msg, Code: code}
}
