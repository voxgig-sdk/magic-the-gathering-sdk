...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MAGICTHEGATHERING_TEST_LIVE") == "TRUE" ||
		os.Getenv("MAGICTHEGATHERING_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MAGICTHEGATHERING_TEST_EXPLAIN"); explain != "" {
