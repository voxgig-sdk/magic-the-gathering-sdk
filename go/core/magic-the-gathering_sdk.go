...
import (
	...
	vs "github.com/voxgig-sdk/magic-the-gathering-sdk/go/utility/struct"
)

type MagicTheGatheringSDK struct {
	Mode     string
...
func (sdk *MagicTheGatheringSDK) OptionsMap() map[string]any {
...
func (sdk *MagicTheGatheringSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *MagicTheGatheringSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *MagicTheGatheringSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *MagicTheGatheringSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *MagicTheGatheringSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *MagicTheGatheringSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("MagicTheGatheringSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *MagicTheGatheringSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *MagicTheGatheringSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("MagicTheGatheringSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *MagicTheGatheringSDK {
	...
	sdk := NewMagicTheGatheringSDK(sdkopts)
	sdk.Mode = "test"
