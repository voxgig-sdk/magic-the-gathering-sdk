...
module MagicTheGatheringUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MagicTheGatheringResponse.new({})
    ...
    if fetch_err
      ...
      response = MagicTheGatheringResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MagicTheGatheringResponse.new(fetched)
    else
