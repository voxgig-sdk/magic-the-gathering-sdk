# MagicTheGathering SDK Card entity
...
class CardEntity
  ...
  def data_set(args)
    if args
      @_data = MagicTheGatheringHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = MagicTheGatheringHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Card, Hash] the loaded Card; raises MagicTheGatheringError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MagicTheGatheringHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Card>, Array] the matching Card items; raises MagicTheGatheringError on failure
