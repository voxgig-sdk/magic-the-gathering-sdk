...
require_relative 'MagicTheGathering_types'
...
class MagicTheGatheringSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MagicTheGatheringHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MagicTheGatheringHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MagicTheGatheringFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MagicTheGatheringUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MagicTheGatheringHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MagicTheGatheringHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MagicTheGatheringHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => MagicTheGatheringError.new(
        "#{op}_allow",
        "MagicTheGatheringSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue MagicTheGatheringError => err
    ...
    if fetched.is_a?(Hash)
      status = MagicTheGatheringHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MagicTheGatheringError.new(
        "graphql_error", "MagicTheGatheringSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = MagicTheGatheringSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
